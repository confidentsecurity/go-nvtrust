...
func (g *GpuAttester) GetRemoteEvidence(nonce int) []RemoteEvidence {
	ret := g.nvmlHandler.Init()

	computeState, ret := g.nvmlHandler.SystemGetConfComputeState()
	if ret != nvml.SUCCESS {
		log.Fatalf("Unable to get compute state: %v", nvml.ErrorString(ret))
	}
	if computeState.CcFeature != nvml.CC_SYSTEM_FEATURE_ENABLED {
		log.Fatalf("Confidential computing is not enabled")
	}
	...
	for i := 0; i < count; i++ {
		...
		if ret != nvml.SUCCESS {
			log.Fatalf("Unable to get architecture of device at index %d: %v", i, nvml.ErrorString(ret))
		}

		if deviceArchitecture != nvml.DEVICE_ARCH_HOPPER {
			log.Fatalf("Device at index %d is not supported", i)
		}

		report, ret := device.GetConfComputeGpuAttestationReport()

		if ret != nvml.SUCCESS {
			log.Fatalf("Unable to get attestation report of device at index %d: %v", i, nvml.ErrorString(ret))
		...
		if ret != nvml.SUCCESS {
			log.Fatalf("Unable to get certificate of device at index %d: %v", i, nvml.ErrorString(ret))
		}
		...
		if err != nil {
			log.Fatalf("Failed to encode certificate chain: %v", err)
		}
